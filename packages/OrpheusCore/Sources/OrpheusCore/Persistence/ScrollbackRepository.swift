...
public actor ScrollbackRepository {
    ...
    private var pendingChunkIndices: [String: Int] = [:]   // next chunk_index to write
    ...
    public func append(terminalID: TerminalID, bytes: Data) async {
        ...
        if pendingBuffers[key]!.count >= ScrollbackConstants.scrollbackChunkSize {
            ...
            await flushBuffer(for: terminalID)
    ...
    private func scheduleFlushTask(for terminalID: TerminalID) {
        ...
        flushTasks[key] = Task { [weak self] in
            ...
            await self.flushBuffer(for: terminalID)
    ...
    private func flushBuffer(for terminalID: TerminalID) async {
        let key = terminalID.rawValue
        ...
        while offset < data.count {
            ...
            offset = end
            await writeChunk(Data(chunk), for: terminalID)
        }
    }

    private func writeChunk(_ chunk: Data, for terminalID: TerminalID) async {
        // Determine the next chunk index to write and enforce the ring limit.
        do {
            ...
            OrpheusLogger.persistence.error(
                "ScrollbackRepository: failed to write chunk for \(terminalID.rawValue, privacy: .public): \(error.localizedDescription, privacy: .public)"
            )
        }
    }
}
