...
- Before the automation-management descriptors landed, the all-enabled
  ...
  default-exposed, 48 runtime-visible, and 5 automation-eligible
...
| Automations             | Durable bounded schedules/events invoke the canonical registry in-process. A fixed exact-workspace grant covers five naturally idempotent operations: effective settings, workspace settings patch, lineage, reopen, and rename. Every grant is matched against server-owned kind, permission, risk, effects, scope, and idempotency metadata; project resource metadata remains MCP-only. Renderer IPC/preload, Settings → Automations, and nine MCP-only exact-bound-workspace operations support definitions, state, runs, and manual retry generations; MCP management is never automation-eligible. | `src/main/automations/`, `src/main/ipc/automations.ts`, `src/main/controlPlane/automationManagementCapabilities.ts`, `src/main/controlPlane/safeAutomationGrants.ts`, `src/renderer/src/components/dashboard/settings/OrpheusAutomationsSection.tsx` |
...
7. **Durable Automations — implemented and live-validated through the Dev QA
   ...
   request/audit ids, restart recovery, disable, and scoped cleanup. That
   packaged-only command fixture has since been removed. Current source
