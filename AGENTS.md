...
- **Project**: a `cwd` registered in `~/Library/.../orpheus.sqlite`. Maps to a `~/.Codex/projects/<encoded-cwd>/` dir (Codex's transcript store; slashes become dashes).
- **Workspace**: an isolated unit of exploration scoped to a project. Each workspace owns **exactly one persistent libghostty surface** keyed by `workspaceId`. Each workspace = one `Codex` session (captured via the `.jsonl` written under the encoded project dir, then passed back as `--resume <sessionId>`). Navigation **hides** the surface (`addon.hide`), never destroys it; full teardown only happens on archive or project removal.
- **Session**: a row sourced from Codex's on-disk `.jsonl` transcript, joined to the workspace via `claude_session_id`. The DB row mirrors metadata; the JSONL is authoritative content.

### Codex launch composition (the central abstraction)

The terminal is `libghostty` running `resources/orpheus-Codex.sh`. The shell wrapper reads two env vars set by `terminal:mount`:

- `ORPHEUS_CLAUDE_FLAGS` — whitespace-split CLI flags
- `ORPHEUS_CLAUDE_SETTINGS_JSON` — inline JSON for `Codex --settings`
...
**When wiring a new Codex setting:** see `.Codex/agents/audit-Codex-env-vars.md` — it is the canonical procedure (schema column → type field → row→record mapping → BOOLEAN_KEYS/validator → `composeClaudeLaunch` emission **before the `customEnvVars` merge** → UI `SettingRow` with `mapsTo` chip → `.Codex/snapshots/env-vars.json` flipped to `wired: true`).
...
- **Settings are layered.** Any UI control that maps to Codex settings must compose through `composeClaudeLaunch` and carry a `mapsTo` chip pointing to the env var or settings key it produces.
- **Commits use Conventional Commits, no emoji.** `feat(scope):`, `fix(scope):`, `chore(scope):`. No `Co-Authored-By: Codex` lines unless explicitly requested.
- **Audit env vars before adding new settings.** The `.Codex/agents/audit-Codex-env-vars.md` agent diffs `https://code.Codex.com/docs/en/env-vars.md` against `.Codex/snapshots/env-vars.json` — run it (or invoke it as a subagent) before guessing whether something is already wired.
