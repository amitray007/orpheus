...
| `bun run build:native`     | Rebuild `packages/ghostty-native` (`node-gyp` against the installed Electron ABI) **and** `packages/terminal-vt-native` (napi-rs, currently blocked — see `Cargo.toml`).                                                                         |
...
`packages/terminal-vt-native/` is the in-progress Rust replacement (napi-rs + libghostty-vt). Currently blocked on Zig 0.15.2 + Apple SDK 26.5 — see the `Cargo.toml` comment block. Don't try to enable it without reading `docs/notes/libghostty-vt-blocker.md`.
