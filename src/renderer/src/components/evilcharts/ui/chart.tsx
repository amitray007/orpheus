...
// evilcharts registry convention: this ui/chart.tsx module exports the
// ChartContainer component alongside its shared context hook + helpers
// (mirrors upstream shadcn's own ui/chart.tsx shape); kept as-is to stay
// diffable against future `shadcn add @evilcharts/chart` re-syncs.
...
function LoadingIndicator({ isLoading }: { isLoading: boolean }): React.JSX.Element | null {
  if (!isLoading) {
    return null
  }
...
const ChartStyle = ({
  ...
  const generateCssVars = (theme: keyof typeof THEMES): string =>
    colorConfig
...
// Helper to extract item config from a payload. evilcharts registry
// convention (see the useChart export above for the full rationale).
// eslint-disable-next-line react-refresh/only-export-components
...
function axisValueToPercentFormatter(value: number): string {
...
// Generate random loading data for skeleton/loading state (min/max represent
// percentage of the range 0-100, defaults to 20-80 for realistic look).
// evilcharts registry convention (see the useChart export above).
// eslint-disable-next-line react-refresh/only-export-components
