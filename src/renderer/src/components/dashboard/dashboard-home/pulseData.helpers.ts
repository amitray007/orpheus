...
// ---------------------------------------------------------------------------
// 6. Models breakdown — session count by model, for the donut. `model` is
//    normalized to a friendly display name (Opus/Sonnet/Haiku) by substring
//    match, matching how model id strings actually look
//    (e.g. "claude-opus-4-1-20260305"). Sessions with a null/empty model are
//    grouped under "Unknown" (dropped only if the WHOLE dataset were empty —
//    otherwise a session that legitimately ran should still count toward
//    the total, just unattributed) rather than silently discarded, so the
//    donut total always equals the tile's Sessions count. Only the top 3
//    models by count are broken out individually; everything else
//    (including "Unknown" if it doesn't make the top 3) collapses into
//    "Other" so the donut/legend never sprawls past 4 slices.
// ---------------------------------------------------------------------------
...
/** Map a raw model id string to a short friendly label. */
...
const MAX_MODEL_SLICES = 3

export function computeModelBreakdown(sessions: SessionRecord[]): ModelSliceData[] {
  const counts = new Map<string, number>()
  for (const s of sessions) {
    const label = s.model ? friendlyModelName(s.model) : 'Unknown'
  ...
  if (sorted.length <= MAX_MODEL_SLICES) return sorted

  const top = sorted.slice(0, MAX_MODEL_SLICES)
  const otherCount = sorted.slice(MAX_MODEL_SLICES).reduce((sum, s) => sum + s.count, 0)
  return [...top, { key: 'Other', label: 'Other', count: otherCount }]
}
