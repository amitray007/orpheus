...
function readRow(): Row | undefined {
  const db = getDb()
  return db
    .prepare('SELECT cloud_provider, auth_api_key, auth_token, auth_base_url FROM claude_global_settings WHERE id = 1')
...
export function updateClaudeAuth(patch: ClaudeAuthPatch): ClaudeAuthState {
  ...
  db.prepare(
    `UPDATE claude_global_settings
     SET cloud_provider = ?, auth_api_key = ?, auth_token = ?, auth_base_url = ?, updated_at = ?
     WHERE id = 1`
  ).run(next.cloud_provider, next.auth_api_key, next.auth_token, next.auth_base_url, now)
